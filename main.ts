...
export default class NextTabGroupPlugin extends Plugin {
	...
	private async rotateTabGroups() {
		...
		// Set the base leaf to the first tab of the first group
		await baseLeaf.setViewState(tabGroups[0][0].viewState, tabGroups[0][0].ephemeralState);
		const firstTabGroup = baseLeaf.parent;
		
		// Add remaining tabs to first group
		for (let i = 1; i < tabGroups[0].length; i++) {
			const newLeaf = this.app.workspace.createLeafInParent(firstTabGroup, i);
			await newLeaf.setViewState(tabGroups[0][i].viewState, tabGroups[0][i].ephemeralState);
		}
		
		console.log(`[next-tab-group] First group populated with ${tabGroups[0].length} tabs`);
		
		// Now create additional groups with the NEW direction
		for (let groupIdx = 1; groupIdx < tabGroups.length; groupIdx++) {
			...
			console.log(`[next-tab-group] Creating group ${groupIdx} with ${splitCommand}`);
			
			// Make sure we have an active leaf before splitting
			...
			if (!newLeaf) {
				console.warn(`[next-tab-group] No active leaf after split ${groupIdx}`);
				continue;
			}
			
			const newTabGroup = newLeaf.parent;
			
			// Populate this group
			await newLeaf.setViewState(group[0].viewState, group[0].ephemeralState);
			
			for (let i = 1; i < group.length; i++) {
				const leaf = this.app.workspace.createLeafInParent(newTabGroup, i);
